...
async function DanhSachKTCD() {
    try {
      // Gọi AJAX để xóa payment
      let response = await fetch("/api/student/getDKDT", {
        method: "POST",
        headers: {
          "Content-Type": "application/x-www-form-urlencoded",
        },
        body: "function=" + encodeURIComponent("DanhSachKTCD"),
      });
      ...
      console.log("Dữ liệu dkdt nhận được từ API:", data);
...
async function btnLoc() {
    try {
        ...
        let response = await fetch("/api/student/getDKDT", {
            ...
            body: `function=DanhSachKTCD&khoaThi=${encodeURIComponent(khoaThi)}&capDai=${encodeURIComponent(capDai)}}`,
        });
        ...
        let data = await response.json();
        console.log("Dữ liệu Lọc dkdt nhận được từ API:", data);
        await showTableKQ(data);
        loadPage();
    } catch (error) {
        console.error(error);
...
async function showTableKQ(data) {
    console.log("Dữ liệu dkdt trong loadData:", data);  // Ghi lại dữ liệu nhận được để kiểm tra
  ...
  document.getElementById('searchForm').addEventListener('submit', function (e) {
    ...
    fetch(`/api/student/getDKDT?TimKiem=${keyword}`)
        ...
        .then(data => {
            ...
            data.forEach(result => {
                let row = `<tr>
                    ...
                    <td>${result.ngayThi}</td>
                    <td>${result.ngayKetThuc}</td>
                    <td><button class="Btndkithi" data-id="${result.maMonSinh}">Đăng kí dự thi</button></td>
                </tr>`;
